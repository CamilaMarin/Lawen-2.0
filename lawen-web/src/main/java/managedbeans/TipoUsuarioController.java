...
public class TipoUsuarioController implements Serializable {
    ...
    @EJB
    private TipoUsuarioFacadeLocal ejbFacade;
    ...
    private TipoUsuarioFacadeLocal getFacade() {
        return ejbFacade;
    }
    ...
    public boolean isAdmin(String rut){
        List <TipoUsuario> elements = getFacade().findAll();
        
        for(TipoUsuario elemento :elements){
            
            if(elemento.getRut().equals(rut) && elemento.getRol().equals("ADMIN"))
                return true;
        }
        return false;
    ...
    public TipoUsuario getTipoUsuario(java.lang.Long id) {
